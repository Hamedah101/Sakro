    ...
    if (scanButton) {
        scanButton.addEventListener("click", () => {
            ...
            setTimeout(() => {
                ...
                showToast("Scan finished. These HTML pages use sample data.");
                setTimeout(() => {
...
function setupStaticSettings() {
    const form = document.getElementById("staticSettingsForm");
    if (!form) return;
    form.addEventListener("submit", (event) => {
        event.preventDefault();
        showToast("Settings save works when you run the Python app (app.py).");
    });
}
